...
@NoArgsConstructor
@ToString
public class HierarchicalDistanceTCloseness extends PrivacyModel {

    /** Parameter for the privacy model */
    private double t;

    /**
     * Optional list of attributes to which HierarchicalDistanceTCloseness will be applied. If left empty all sensitive
    ...
    public Collection<PrivacyCriterion> getPrivacyCriterion(Data data) {
        ...
        for (String sensitiveAttribute : sensitiveAttributes) {
            privacyCriterionList.add(new org.deidentifier.arx.criteria.HierarchicalDistanceTCloseness(sensitiveAttribute, t, data.getDefinition().getHierarchyObject(sensitiveAttribute)));
        }
